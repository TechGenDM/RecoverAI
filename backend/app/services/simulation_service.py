...
# Base epoch for simulation clock to ensure complete independence from wall-clock time
SIM_EPOCH = datetime(2024, 1, 1, 0, 0, 0, tzinfo=UTC)

# Core deterministic scenarios for indices 0-7
CORE_SCENARIOS = [
    {"reason": "insufficient_funds", "email": "sim+1@example.com", "phone": "9999999991", "amount": 150000},
    {"reason": "invalid_otp", "email": "sim+2@example.com", "phone": "9999999992", "amount": 25000},
    {"reason": "gateway_technical_error", "email": "sim+3@example.com", "phone": "9999999993", "amount": 500000},
    {"reason": "card_declined", "email": "sim+4@example.com", "phone": "9999999994", "amount": 75000},
    {"reason": "fraud_suspected", "email": "sim+5@example.com", "phone": "9999999995", "amount": 1000000},
    {"reason": "network_error", "email": "sim+6@example.com", "phone": "9999999996", "amount": 12500},
    {"reason": "insufficient_funds", "email": None, "phone": None, "amount": 200000},
    {"reason": "gateway_technical_error", "email": "sim+8@example.com", "phone": None, "amount": 350000},
]

# Random scenario generation pool
REASONS = ["insufficient_funds", "invalid_otp", "gateway_technical_error", "card_declined", "fraud_suspected", "network_error"]
...
def get_deterministic_hash(seed: int, scenario_index: int) -> float:
    ...
    key = f"sim_{seed}_{scenario_index}".encode("utf-8")
...
async def run_simulation(session: AsyncSession, seed: int, scenario_count: int) -> dict:
    ...
    logger.info(f"Starting simulation run with seed={seed}, count={scenario_count}")
    ...
    for index in range(scenario_count):
        ...
        if existing_payment:
            already_existed += 1
            case_stmt = select(RecoveryCase).where(RecoveryCase.original_payment_id == payment_id)
            case = (await session.execute(case_stmt)).scalar_one_or_none()
            if not case:
                logger.error(f"Payment {payment_id} exists but no RecoveryCase found!")
                continue
            ...
            payment = Payment(
                ...
                error_source="customer" if scenario["reason"] in ["insufficient_funds", "invalid_otp"] else "gateway",
            ...
            # Create Case
            window_expires = failed_at + timedelta(hours=settings.RECOVERY_MAX_WINDOW_HOURS)
            case = RecoveryCase(
...
async def process_simulation_case(session: AsyncSession, case_id: str, seed: int, index: int):
    ...
    if case.status in ["CREATED", "WAITING"]:
        # Only process WAITING if due_at is past the simulation evaluation_time (which is SIM_EPOCH)
        ...
        try:
            ...
            logger.error(f"Simulation M2 failed for case {case.id}: {e}")
            await session.rollback()
            raise # Raise to fail the test and show the error!
            
    # M3 Execution Phase
    case = await session.get(RecoveryCase, case_id)
    if case.status == "EXECUTING" or case.status == "ANALYSING": 
    ...
    if case.status == "LINK_SENT":
        ...
        # E.g. 30% recovery rate for simulation
        is_recovered = outcome_h < 0.30
        ...
        ).order_by(RecoveryAction.attempt_number.desc()).limit(1)
        action = (await session.execute(action_stmt)).scalar_one_or_none()
        
        if is_recovered:
            recovered_payment_id = f"pay_sim_rec_{seed}_{index}"
            
            # Check if payment already exists (idempotency safety)
            pay_stmt = select(Payment).where(Payment.razorpay_payment_id == recovered_payment_id)
            if not (await session.execute(pay_stmt)).scalar_one_or_none():
