...
async def build_recovery_context(session: AsyncSession, case: RecoveryCase) -> RecoveryContext:
    ...
    hist_stmt = (
        select(
            ...
            func.sum(
                sql_case((Payment.status == 'captured', 1), else_=0)
            ).label('success_count'),
            func.sum(
                sql_case((Payment.status == 'failed', 1), else_=0)
            ).label('failed_count'),
    ...
    repeated_stmt = select(func.count(Payment.id)).where(
        ...
        Payment.created_at >= (now.timestamp() - 7*86400) # Last 7 days roughly, though created_at is datetime
    )
    ...
    for i, a in enumerate(actions):
        prev_actions.append(PreviousAction(
            attempt_number=i+1,
    ...
    recovery_history = RecoveryHistory(
        attempt_count=len(prev_actions),
