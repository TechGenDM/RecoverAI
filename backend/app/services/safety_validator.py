...
def compute_heuristic_likelihood(error_reason: str | None) -> float:
    ...
    return RECOVERY_LIKELIHOOD_HEURISTICS.get(error_reason.lower(), DEFAULT_HEURISTIC)


def validate_decision(
    decision: RecoveryDecisionSchema, context: RecoveryContext
) -> SafetyResult:
    ...
    if action == "WAIT" and delay_hours > context.time.hours_remaining:
        ...
        return SafetyResult(
            ...
            policy_modification_detail={
                "original_delay": delay_hours,
                "new_delay": adjusted_delay,
            },
