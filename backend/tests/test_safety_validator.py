...
def get_dummy_context(
    window_expired=False, can_send_link=True, attempt_number=1, hours_remaining=24.0
):
    ...
    return RecoveryContext(
        ...
        historical=HistoricalContext(
            ...
            failed_payments=1,
            success_rate=None,
            has_previously_paid=False,
            has_previously_failed=True,
            avg_payment_amount=None,
            days_since_last_payment=None,
            repeated_method_failures=0,
        ),
        recovery_history=RecoveryHistory(
            attempt_count=attempt_number - 1, previous_actions=[]
        ),
        time=TimeContext(
            failed_at=datetime.now(UTC).isoformat(),
            evaluation_time=datetime.now(UTC).isoformat(),
            recovery_window_expires_at=(
                datetime.now(UTC) + timedelta(hours=hours_remaining)
            ).isoformat(),
