...
async def test_analyze_and_decide(db_session, setup_test_data):
    ...
    assert decision.recommended_action in ["WAIT", "SEND_PAYMENT_LINK", "ESCALATE", "STOP"]
    assert decision.llm_provider is not None
    assert decision.policy_verdict in ["ALLOW", "MODIFY", "DENY"]
    assert decision.effective_action == case.status if case.status != "ANALYSING" else "SEND_PAYMENT_LINK"

    # 5. Check Audit Event
    audit = (await db_session.execute(
        select(AuditEvent)
        .where(AuditEvent.case_id == case.id, AuditEvent.event_type == "LLM_ANALYSIS_COMPLETED")
    )).scalar_one_or_none()
